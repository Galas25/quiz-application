...
export default function InstructorDashboard() {
  // Added setResults and currentUser to the destructuring
  const { results, setResults, releaseScores, released, setCurrentUser, currentUser } = useContext(QuizContext);
  ...
  const clearAllData = () => {
    if (window.confirm("Are you sure? This will delete all student submissions for the demo.")) {
      setResults([]); // Now this will work because it's imported from Context
  ...
  const flaggedCount = results?.filter(r => r?.violations > 0).length || 0;
                            ...
                            <td className="px-6 py-4">
                              {released ? (
                                <div className="flex flex-col">
                                  <span className="font-bold text-slate-800 text-sm">{score} / {questions.length}</span>
